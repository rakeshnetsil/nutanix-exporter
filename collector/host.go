...
func (e *HostExporter) Describe(ch chan<- *prometheus.Desc) {
	...
	e.CpuCores = prometheus.NewGaugeVec(prometheus.GaugeOpts{Namespace: hostNamespace, Name: "num_cpu_cores", Help: "Total number of cpu cores on each host"}, hostLabels)
...
func (e *HostExporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, s := range hosts {
		...
		{
			g := e.CpuCores.WithLabelValues(s.Name)
			g.Set(float64(s.CpuCores))
			g.Collect(ch)
		}
